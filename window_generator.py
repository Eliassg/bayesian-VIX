...
class WindowGenerator():
  '''
  Window generator object which is the backbone in the time series forecasting. Makes the dataset with a temporal structure.
  Creates folds for hyperparameter tuning. 
  Based on https://www.tensorflow.org/tutorials/structured_data/time_series#recurrent_neural_network
  n_splits is the number of folds for cross  validation
  train_splits, test_splits gives the proportion within each fold, (3,1) gives 0.75% train
  The windowgenerator ensures that the scaling is performed correctly, i.e. fit on train set only for every fold.
  '''
  def __init__(self, input_width, label_width, shift,
               train_df=None, val_df=None, test_df=None,
               label_columns=None, n_splits=5, train_splits=3, test_splits=1,
               scale=True, scaler=MinMaxScaler):
